...
3.  **Prerequisite:** The `townName` specified in the suggestion **must** already exist as a document in the `towns` collection in Firestore. If the town doesn't exist, the approval process might show an error or the location might not display correctly. Add the town first via Firestore (see section 5.2).
...
3.  **To Edit a Suggestion before Approval:**
    *   Directly modify any fields (e.g., `name`, `description`, `category`) in the Firestore document. Save the changes. Then you can approve it via the admin panel.
...
*   **Adding a New Town:**
    ...
    4.  **Required Fields:**
        *   `name` (String): e.g., "Formby"
        *   `county` (String): e.g., "Merseyside"
        *   `country` (String): e.g., "UK"
        *   `coordinates` (Map):
            *   `lat` (Number): Latitude, e.g., `53.558`
            *   `lng` (Number): Longitude, e.g., `-3.067`
        *   `description` (String): A brief description of the town.
        *   `imageUrl` (String, Optional): URL to the town's banner image stored in Firebase Storage. (e.g., `gs://<your-bucket-name>/town-images/formby.png` or the public HTTPS URL).
...
*   These documents are typically created when a suggestion is approved.
